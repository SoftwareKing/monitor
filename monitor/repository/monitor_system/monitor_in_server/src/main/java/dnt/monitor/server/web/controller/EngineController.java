 ...
 * <pre>
 * <b>HTTP    URI                         方法      含义  </b>
 *  POST     /engine/self                 register 引擎安装好第一次启动时，向服务器注册自身
...
class EngineController extends SouthController {
    ...
    public MonitorEngine register(@Valid @RequestBody MonitorEngine engine){
        logger.info("Registering {}", engine);
        try {
            MonitorEngine created = engineService.register(engine);
