...
/**
 * <h1>资源策略被更新后，更新所有的监控引擎</h1>
 */
@Component
class UpdatePolicy2EngineAfterCreated extends Bean
        implements ApplicationListener<ObjectUpdatedEvent<ResourcePolicy>> {
    @Autowired
    ServiceLocator       serviceLocator;
    @Autowired
    EngineServiceLocator engineServiceLocator;

    public UpdatePolicy2EngineAfterCreated() {
        setOrder(100);
    }

    @Override
    public void onApplicationEvent(ObjectUpdatedEvent<ResourcePolicy> event) {
        ...
        for (MonitorEngine engine : engines) {
            ...
            try {
                ...
                configurationService.updatePolicy(policy);
            } catch (OfflineException e) {
                logger.debug("{} has been updated to an offline {}", policy, engine);
            } catch (EngineException e) {
                logger.warn("Can't update {} to {}", policy, engine);
            }
        }
