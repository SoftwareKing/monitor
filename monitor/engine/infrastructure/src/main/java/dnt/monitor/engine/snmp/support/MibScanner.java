 ...
 * @author Jay Xiong
 */
@ManagedResource(objectName = "dnt.monitor.engine:name=mibScanner")
@Component
class MibScanner extends ApplicationSupportBean implements ApplicationListener<SystemStartedEvent> {
    @Autowired // for local auto-wired
    private MibRepository           repository;
    @Autowired
    @Qualifier("globalClassLoader")
    ClassLoader globalClassLoader;

    private String                  name;
    private String   preLoad = "classpath*:mibs/**";
    private String[] load    = new String[]{"SNMP*", "RFC*", "CISCO*"};
    ...
    public void onApplicationEvent(SystemStartedEvent event) {
        Resource[] resources;
        try {
            PathMatchingResourcePatternResolver resourceLoader = new PathMatchingResourcePatternResolver(globalClassLoader);
            resources = resourceLoader.getResources(getPreLoad());
