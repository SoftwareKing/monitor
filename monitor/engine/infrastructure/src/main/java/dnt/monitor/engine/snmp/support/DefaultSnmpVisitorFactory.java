 ...
 */
@Component
class DefaultSnmpVisitorFactory implements SnmpVisitorFactory{
    //系统启动之后，会自动将该对象替换为DefaultMibRepository
    @Autowired
    MibRepository repository;


    public MibAwareSnmpVisitor visitor(ResourceNode node, SnmpCredential credential) {
        //TODO 复用这个对象
        Target target = createTarget(node.getResource().getAddress(), credential);
        ...
        return new FullSnmpVisitor(target, getSnmp(), repository);
    }
